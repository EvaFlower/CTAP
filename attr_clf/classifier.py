...
imgIndex = np.load("../URCGAN_Rel/imgIndex_smiling.npy", allow_pickle=True)[:-1]
imgAttr = np.load("../URCGAN_Rel/anno_dic_smiling.npy", allow_pickle=True).item()
print(len(imgIndex), len(imgAttr))
imgs = read_celebahq_lmdb(num_imgs)
...
for name, n_cat in params.attr:
    for i in range(n_cat):
        attrs.append(torch.FloatTensor((targets == i).astype(np.float32)))
targets = torch.cat([x.unsqueeze(1) for x in attrs], 1)
print(imgs.shape, targets.shape, targets[:10])
#imgs = imgs/127.5-1
print(np.max(imgs), np.min(imgs))
X_train = imgs[:num_train]
...
valid_data = DataSampler(X_test, y_test, params)
test_data = DataSampler(X_test, y_test, params)

#data, attributes = load_images(params)
#data = load_images_from_path()
...
#print(data.shape, attributes.shape)
#print(attributes[0])
...
#train_data = DataSampler(data, attributes, params)
#valid_data = DataSampler(data, attributes, params)
#test_data = DataSampler(data, attributes, params)
