"""
Some codes from https://github.com/carpedm20/DCGAN-tensorflow/blob/842dd27aaa06c5280dbdccdff9daa2e5ce6f595f/utils.py
"""
...
def imread(path, grayscale=False):
    if (grayscale):
        ...
        img_rgb = img_bgr[..., ::-1]
        return img_rgb.astype(np.uint8) #img_bgr.astype(np.uint8) #
...
def center_crop(x, crop_h, crop_w,
    ...
    return im #np.array(im.resize([resize_h, resize_w]), np.uint8(Image.BILINEAR))
...
def transform(image, input_height, input_width,
    ...
    #im = 2 * (np.array(im.resize([resize_height, resize_width])) / 255. - 0.5)
    im = np.array(im.resize([resize_height, resize_width]))
    # imagenet_mean = np.array([0.5, 0.5, 0.5], dtype=np.float32) * 255.0
    # imagenet_std = np.array([0.5, 0.5, 0.5], dtype=np.float32) * 255.0
