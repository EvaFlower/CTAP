...
import pandas as pd
#from keras-adamw.keras_adamw import AdamW
...
class Relgan():
    ...
    def __init__(self, args):
        ...
        self.result_path = 'final_results/l2svm_celebahq_finalF_{}_interG_pretrain{}_u{}_s{}_r{}_i{}_gp{}_im{}_ss{}_ni{}_bs{}_ug{}_stop{}_gl{}_dl{}_zp{}_wz{}_cls'.\
            format(self.attr_name, self.pretrain, self.lambda_u, self.lambda_s, self.lambda_r, self.lambda_i, self.gp_l, \
            self.imgSize, self.save_iter_step, self.num_imgs, self.batch, self.lambda_ug, self.stop, self.g_lr, self.d_lr, self.zero_p, self.with_zero)
    ...
    def get_model(self):
        ...
        dis_path = self.load_model_path+'/model/discriminator50.h5'
        gen_path = self.load_model_path+'/model/generator50.h5'
        if self.pretrain:
    ...
    def get_loss(self):
        ...
        self.gc_loss_real, self.gc_loss_fake = lsuggan([dc_real, dc_fake], [zeros, K.sign(self.vec_ag)], ones)
    ...
    def get_optimizer(self):
        ...
        #d_opt = AdamW(lr=self.lr, beta_1=self.b1, beta_2=self.b2, decay=self.lambda_w, batch_size=self.batch, samples_per_epoch=self.num_imgs, epochs=50)
...
#                         image = image*255
                ...
                try:
                    ...
                    print('Pass save')
            #if ite%int(self.save_iter_step)==0 and ite>0:
            #    stop_idx += 1
            #    cls_acc = self.val_acc()
            #    if self.best_cls_acc < cls_acc:
            #        stop_idx = 0
            #        print('save best model with acc {} at {}'.format(cls_acc, ite))
            #        self.best_cls_acc = cls_acc
            #        self.g_model.save(self.model_path+"/bestgenerator.h5")
            #        self.d_model.save(self.model_path+"/bestdiscriminator.h5")
            #if stop_idx > 9 and self.stop:
            #    #print('changing smaller lr')
            #    
            #    print('stop training at iteration ', str(ep))
            #    break 
            #te = ite + 1
