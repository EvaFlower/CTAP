...
class Trainer(object):
    ...
    def lat_dis_step(self):
        ...
        self.stats['lat_dis_costs'].append(loss.data.cpu().numpy())
    ...
    def autoencoder_step(self):
        ...
        self.stats['rec_costs'].append(loss.data.cpu().numpy())
...
def classifier_step(classifier, optimizer, data, params, costs):
    ...
    # loss / optimize
    loss = get_attr_loss(preds, batch_y, False, params)
    #costs.append(loss.data[0]) pytorch<0.5
    costs.append(loss.data.cpu().numpy())
    optimizer.zero_grad()
    loss.backward()
    if params.clip_grad_norm:
