...
parser.add_argument("-n", "--number", type=int, default=1, help="attribute number")
parser.add_argument("-o", "--output", type=str, default='anno_dic_High_Cheekbones.npy', help="output file")
args = parser.parse_args()

annos = open('/data/yinyao/Projects/data/face_dataset/CelebA/Anno/list_attr_celeba.txt').readlines()
...
if args.number == 17:
    ...
    #new_attrs = ['Mustache']
    #new_attrs = ['Male']
    #new_attrs = ['Mouth_Slightly_Open']
    new_attrs = ['Arched_Eyebrows']
    new_attrs = ['Big_Lips']
    new_attrs = ['Heavy_Makeup']
    new_attrs = ['Attractive']
    new_attrs = ['Wavy_Hair']
    new_attrs = ['Wearing_Lipstick']
    new_attrs = ['High_Cheekbones']
        
elif args.number == 2:
    new_attrs = ['Smiling', 'Young']
else:
...
print(len(annosAry))
#print(np.max(annosAry), np.min(annosAry))
...
np.save("imgIndex_High_Cheekbones.npy", imgIndex)
